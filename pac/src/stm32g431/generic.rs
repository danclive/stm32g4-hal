...
mod atomic {
    ...
    macro_rules! impl_atomics {
        ($U:ty, $Atomic:ty) => {
            impl AtomicOperations for $U {
                unsafe fn atomic_or(ptr: *mut Self, val: Self) {
                    unsafe { (*(ptr as *const $Atomic)).or(val, Ordering::SeqCst) };
                }

                unsafe fn atomic_and(ptr: *mut Self, val: Self) {
                    unsafe { (*(ptr as *const $Atomic)).and(val, Ordering::SeqCst) };
                }

                unsafe fn atomic_xor(ptr: *mut Self, val: Self) {
                    unsafe { (*(ptr as *const $Atomic)).xor(val, Ordering::SeqCst) };
                }
            }
        };
    }
        ...
        REG::Ux: AtomicOperations,
    {
            ...
            F: FnOnce(&mut W<REG>) -> &mut W<REG>,
        {
            ...
            .bits;
            unsafe { REG::Ux::atomic_or(self.register.as_ptr(), bits) };
        }
            ...
            F: FnOnce(&mut W<REG>) -> &mut W<REG>,
        {
            ...
            .bits;
            unsafe { REG::Ux::atomic_and(self.register.as_ptr(), bits) };
        }
            ...
            F: FnOnce(&mut W<REG>) -> &mut W<REG>,
        {
            ...
            .bits;
            unsafe { REG::Ux::atomic_xor(self.register.as_ptr(), bits) };
        }
    }
}
